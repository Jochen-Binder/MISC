...
class verticalize(object):
    ...
    def pivot(self, df, variable='variable', value='value', ID = 'tmp_ID'):
        cols = [c for c in df.columns if c not in {variable, value}]
        df = df.set_index(cols + [variable])
        ...
        df1 = df[cols]
        df2 = df[[variable, value, 'brand', ID]]
        df2 = df2.set_index([ID,  'brand', 'variable'])
        df2 = df2.unstack(variable)
        ...
        df_tmp = df_tmp.groupby(['tmp_ID', 'variable']).max().unstack(variable)
        
        tmp = pd.DataFrame()
        counter = 0
        
        for k in names:
            df_tmp = df.loc[df[ID]==k,:]
            df_tmp = df_tmp.groupby(['tmp_ID', 'variable']).max().unstack(variable)
            df_tmp.columns = df_tmp.columns.levels[1]
            df_tmp.columns.name = None
            tmp = pd.concat([tmp, df_tmp], axis=0)
            counter += 1
            print ("{:.0f}".format(counter/len(names)*100),'%')
        df = tmp.copy()
        del tmp
        df = df.reset_index()
        
        def reduce_columns(n):
            if len(names[0:k])*len(df.columns) > 300000000:
                df
