...
class ipfn(object):
    ...
    def __init__(self, df, aggregates, dimensions, weight_col='total',
    ...
    def weighting(self):
        ...
        Runs the ipfn algorithm. Automatically detects of working with numpy ndarray or pandas dataframes.
        """      
        self.df_n = self.iteration()
        df_goal = self.df_n.groupby(self.unique_dimensions)['total'].sum().reset_index()
        df_origin = self.original.groupby(self.unique_dimensions)['total'].sum().reset_index()
        
        tmp = pd.merge(df_origin, df_goal, how = 'left', on=self.unique_dimensions, suffixes=('_origin', '_goal'), sort=False)
        tmp['weight'] = tmp['total_goal']/tmp['total_origin']

        df_weighted = self.df.merge(tmp[self.unique_dimensions+['weight']], on=self.unique_dimensions, sort = False)
...
if __name__ == '__main__':
    ...
    ipfn_k = ipfn(data, [agg0, agg1, agg2],
                   [['STATUS_TN'], ['qd1'], ['STATUS_TN', 'qd1']], 'total')
    hello = ipfn_k.iteration()
    hello = ipfn_k.weighting()
    


    ipfn_k = ipfn(data, [agg0, agg1],
                   [['STATUS_TN'], ['qd1']], 'total')
    
    
    print(df)
    print(hello.groupby(['STATUS_TN', 'qd1'])['total'].sum(), agg1)

    print(hello.groupby(['qd1'])['total'].sum(), agg1)



# Next steps

print(hello.groupby(['STATUS_TN', 'qd1'])['total'].sum(), ipfn_k.original.groupby(['STATUS_TN', 'qd1'])['total'].sum())
hello_goal = hello.groupby(['STATUS_TN', 'qd1'])['total'].sum().reset_index()
hello_origin = ipfn_k.original.groupby(['STATUS_TN', 'qd1'])['total'].sum().reset_index()

new = pd.merge(hello_origin, hello_goal, how = 'left', on=['STATUS_TN', 'qd1'], suffixes=('_origin', '_goal'), sort=False)
new['weight'] = new['total_goal']/new['total_origin']

results = data.merge(new[flat_list+['weight']], on=['STATUS_TN', 'qd1'], sort = False)

flat_list = np.unique([item for sublist in d for item in sublist]).tolist()
